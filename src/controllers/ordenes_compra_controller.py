...
class OrdenesCompraController:
    ...
    def crear_orden(self, folio: str, detalle: list[dict] | None = None,
        ...
        # Misma lógica que el recibo y la recepción: si hay precios por talla,
        # el total es Σ(pares × precio_talla); si no, cantidad × precio_unitario.
        total = sum(_subtotal_detalle_oc(d) for d in (detalle or []))

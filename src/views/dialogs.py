...
from PySide6.QtWidgets import (
    QAbstractSpinBox, QApplication, QCheckBox, QComboBox, QDateEdit, QDialog, QDoubleSpinBox,
...
def _subtotal_detalle(d: dict) -> float:
    """Subtotal del renglón: Σ(pares × precio) por punto si hay precios por talla;
    si no, cantidad × precio_unitario."""
    puntos = d.get("puntos", []) or []
    con_precio = [t for t in puntos if float(t.get("precio", 0) or 0) > 0]
    if con_precio:
...
class DialogMatrizTallas(QDialog):
    ...
    def _actualizar_total(self) -> None:
        ...
        importe = sum(self.spn_puntos[pid].value() * self.spn_precios[pid].value()
                      for pid in self.spn_puntos)
    ...
    def get_precios(self) -> dict[int, float]:
        return {punto_id: spn.value() for punto_id, spn in self.spn_precios.items()
                if spn.value() > 0}
...
class DialogOrdenCompra(QDialog):
    def __init__(self, controller: OrdenesCompraController, tipo: str = "orden") -> None:
        ...
        self.setModal(True)
        self._puntos_fila: dict[int, dict[int, dict]] = {}
        self._setup_ui()
    ...
    def _configurar_tallas(self, row: int) -> None:
        prev = self._puntos_fila.get(row, {})
        dlg = DialogMatrizTallas(
            self.controller,
            inicial={pid: v["pares"] for pid, v in prev.items()},
            precios_iniciales={pid: v["precio"] for pid, v in prev.items()},
        )
        if dlg.exec() == QDialog.Accepted:
            matriz = dlg.get_matriz()
            precios = dlg.get_precios()
            self._puntos_fila[row] = {
                pid: {"pares": matriz[pid], "precio": precios.get(pid, 0.0)}
                for pid in matriz
            }
        ...
        for row in range(self.table_detalle.row_count()):
            ...
            if cantidad > 0:
                detalle.append({
                    ...
                    "precio": precio,
                    "puntos": [
                        {"punto_id": pid, "pares": v["pares"], "precio": v["precio"]}
                        for pid, v in puntos.items()
                    ],
                })
        if not detalle:
...
class DialogVerOrden(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        for d in detalle:
            ...
            texto_puntos = ", ".join(
                f"P{t['punto']}: {t['pares']}"
                + (f" (${float(t['precio']):,.2f})" if float(t.get('precio', 0) or 0) > 0 else "")
                for t in puntos) if puntos else "—"
            registros.append({
                "insumo_nombre": d.get("insumo_nombre", ""),
                "proveedor_nombre": d.get("proveedor_nombre", "") or "—",
                "puntos_texto": texto_puntos,
                "cantidad": d.get("cantidad", 0),
                "precio_unitario": f"${d.get('precio_unitario', 0):.2f}",
                "subtotal": f"${_subtotal_detalle(d):.2f}",
            })
        self.table.set_records(registros)

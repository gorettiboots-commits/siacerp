...
class _TabPuntos(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        self.grid = ComplexGrid()
        self.grid.set_columnas([
            {"key": "punto", "titulo": "Punto", "ancho": 220},
            {"key": "orden", "titulo": "Orden", "ancho": 120, "tipo": "numero"},
        ])
        self.grid.set_renderers(estilo=self._estilo)
        self.grid.set_exportar_visible(
            tiene(self.permisos, "configuracion", "exportar"))
    ...
    def _editar(self) -> None:
        ...
        if rec is None:
            ...
            return
        dlg = _DialogPunto(self.controller, rec["id"])
        if dlg.exec() == QDialog.Accepted:
            self.recargar()

    def _registros_seleccionados(self) -> list:
        recs = self.grid.registros_seleccionados()
        if not recs:
            QMessageBox.information(self, "Seleccione", "Seleccione al menos un punto de la lista.")
        return recs

    def _desactivar(self) -> None:
        recs = self._registros_seleccionados()
        if not recs:
            return
        nombres = ", ".join(r["punto"] for r in recs)
        resp = QMessageBox.question(
            self, "Confirmar",
        ...
        if resp == QMessageBox.Yes:
            for r in recs:
                self.controller.desactivar_punto(r["id"])
            self.recargar()
    ...
    def _activar(self) -> None:
        ...
        if not recs:
            return
        for r in recs:
            self.controller.activar_punto(r["id"])
        self.recargar()

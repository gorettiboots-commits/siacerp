...
def supabase_upsert(tabla, registros, config):
    url, key = config
...
def sincronizar_tallas(db, config):
    ...
    if registros:
        supabase_upsert("tallas_catalogo_movil", registros, config)
    print_ok(f"{len(registros)} tallas")
    return len(registros)


def sincronizar_insumos(db, config):
    ...
    if registros:
        supabase_upsert("insumos_movil", registros, config)
    print_ok(f"{len(registros)} insumos")
    return len(registros)


def sincronizar_ocs(db, config):
    ...
    if registros:
        supabase_upsert("ordenes_compra_movil", registros, config)
    print_ok(f"{len(registros)} OCs")
    return len(registros)


def sincronizar_ops(db, config):
    ...
    if registros:
        supabase_upsert("ordenes_produccion_movil", registros, config)
    print_ok(f"{len(registros)} OPs")
    return len(registros)


def sincronizar_seguimiento(db, config):
    ...
    if registros:
        supabase_upsert("seguimiento_produccion_movil", registros, config)
...
def main():
    ...
    try:
        ...
        total += sincronizar_ocs(db, config)
        total += sincronizar_ops(db, config)
